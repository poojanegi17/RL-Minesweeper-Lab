...
- **`board`/`mines` fall back to the `"5x5"`/`5` benchmark constant** (`app/config.py`) only when a summary doesn't record its own `rows`/`cols`/`mines` — `dqn_experiment.py`/`ppo_experiment.py` now write those into every summary they produce (`results_loader.py`'s `_build_record`), so a run at a non-default board size reports its real dimensions; only pre-existing summaries from before that change rely on the fallback.
